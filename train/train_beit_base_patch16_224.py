...
from ..scheduler.cosin_sche_warmup import CosineAnnealingWarmUpRestarts
from ..tensorboard.beit_tensor import write_tensorboard
...
num_classes=2
batch_size = 30
file_name = '20220323_beit_base_patch16_224_in22k_e'+str(Epoch)+'_opti_SGD_lrcosgam.8_'+str(Learning_rate)+'up_10_bat_'+str(batch_size)+'_autoaug_1size_imgn_clip_0.1'
device_num = 0

device = torch.device("cuda:"+str(device_num) if torch.cuda.is_available() else "cpu")
CCpu = torch.device('cpu')

model = timm.create_model('beit_base_patch16_224_in22k', pretrained=True, num_classes=num_classes)
...
scheduler = CosineAnnealingWarmUpRestarts(optimizer_ft, T_0=100, T_mult=1, eta_max=0.0001,  T_up=10, gamma=0.8)
...
image_datasets['train'] = train_data # 합친 데이터셋을 train 딕셔너리에 넣음
...
def train_model(model, criterion, optimizer, scheduler, path, writer,  num_epochs=25, result=result):
    since = time.time()
...
model_ft, optimizer_ft = train_model(model, criterion, optimizer_ft, scheduler, total_path, writer,  num_epochs=Epoch, result=result) # 모델 훈련
