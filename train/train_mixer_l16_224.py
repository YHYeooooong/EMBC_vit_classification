...
from ..tensorboard.mixer_L_tensroboard import write_tensorboard
...
num_classes=2
batch_size = 10
file_name = '20220328_mixer_l16_224_in21k_e'+str(Epoch)+'_opti_SGD_lr_oneGitcos02'+str(Learning_rate)+'_up10_bat_'+str(batch_size)+'_autoaug_imgn_clip0.1_lastTTA'
device_num = 0

device = torch.device("cuda:"+str(device_num) if torch.cuda.is_available() else "cpu")
CCpu = torch.device('cpu')

model = timm.create_model('mixer_l16_224_in21k', pretrained=True, num_classes=num_classes)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            if phase == 'train':
                ...
                model.eval()
                model = tta.ClassificationTTAWrapper(model.eval(), tta.aliases.d4_transform())   # 모델을 평가 모드로 설정
                print('TTA다!!')
...
best_model_wts, model_ft, optimizer_ft = train_model(model, criterion, optimizer_ft, scheduler, total_path, writer,  num_epochs=Epoch, result=result) # 모델 훈련
