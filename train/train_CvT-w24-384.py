...
from ..tensorboard.Cvt_tensorboard import write_tensorboard
...
num_classes=2
batch_size = 4
file_name = '20220329_CvT-w24-384_in21k_e'+str(Epoch)+'_opti_SGD_lr_oneGitcos0.0012'+str(Learning_rate)+'_up10_bat_'+str(batch_size)+'_autoaug_imgn_clip0.1'
device_num = 0

device = torch.device("cuda:"+str(device_num) if torch.cuda.is_available() else "cpu")
CCpu = torch.device('cpu')

model = torch.load('../ref_model/whole_CvT-w24-384x384-IN-22k_img22_2class.pt')
...
scheduler = CosineAnnealingWarmUpRestarts(optimizer_ft, T_0=300, T_mult=1, eta_max=0.001,  T_up=10, gamma=0.8)
